...
class Sample:
    def __init__(self):
        ...
        rospy.wait_for_service("/camera/zivid_camera/capture", 30.0)

        rospy.Subscriber("/camera/zivid_camera/points", PointCloud2, self.on_points)

        self.capture_service = rospy.ServiceProxy("/camera/zivid_camera/capture", Capture)

        rospy.loginfo("Enabling the reflection filter")
        general_config_client = dynamic_reconfigure.client.Client(
            "/camera/zivid_camera/capture/general/"
        )
        ...
        frame0_config_client = dynamic_reconfigure.client.Client(
            "/camera/zivid_camera/capture/frame_0"
        )
